...
function ProtectedRoute({ component: Component }: { component: React.ComponentType }) {
  ...
  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center bg-background"><div className="text-center space-y-4"><div className="h-12 w-12 animate-spin rounded-full border-4 border-primary border-t-transparent mx-auto" /><p className="text-muted-foreground">Loading...</p></div></div>;
  }
  if (isError) {
    return <div className="min-h-screen flex items-center justify-center bg-background px-4"><div className="max-w-md text-center space-y-4"><h1 className="text-2xl font-bold">{es ? "No podemos conectar con MyEasyPass ahora mismo" : "We can't reach MyEasyPass right now"}</h1><p className="text-muted-foreground">{es ? "Tu cuenta está bien, solo no pudimos cargarla. Suele durar poco." : "Your account is fine - we just couldn't load it. This is usually brief."}</p><div className="flex flex-wrap justify-center gap-3"><Button onClick={() => retry()}>{es ? "Reintentar" : "Try again"}</Button><Button variant="outline" asChild><a href="/">{es ? "Ir al inicio" : "Go to home page"}</a></Button></div></div></div>;
  }
  if (!isAuthenticated) {
    return <div className="min-h-screen flex items-center justify-center bg-background px-4"><div className="text-center space-y-4"><div className="h-12 w-12 animate-spin rounded-full border-4 border-primary border-t-transparent mx-auto" /><h1 className="text-lg font-medium">{es ? "Te llevamos a iniciar sesión…" : "Taking you to sign in…"}</h1><p className="text-sm text-muted-foreground"><a href="/login" className="text-primary underline-offset-2 hover:underline">{es ? "Continuar a iniciar sesión" : "Continue to sign in"}</a></p></div></div>;
  }
  return <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-background"><div className="text-center space-y-4"><div className="h-12 w-12 animate-spin rounded-full border-4 border-primary border-t-transparent mx-auto" /><p className="text-muted-foreground">Loading...</p></div></div>}><Component /></Suspense>;
}

function HomePage() {
  const { isAuthenticated, isLoading, isError } = useAuth();
  if (isLoading && !isError) return <div className="min-h-screen flex items-center justify-center bg-background"><div className="text-center space-y-4"><div className="h-12 w-12 animate-spin rounded-full border-4 border-primary border-t-transparent mx-auto" /><p className="text-muted-foreground">Loading...</p></div></div>;
...
function App() {
  return <QueryClientProvider client={queryClient}><ThemeProvider defaultTheme="light" storageKey="easy-pass-theme"><TooltipProvider><Toaster /><Router /></TooltipProvider></ThemeProvider></QueryClientProvider>;
}
