...
import {
  Target, Layers, RotateCcw, ClipboardCheck, Sparkles, ChevronRight,
  BookOpen, CalendarDays, FileText, Home, Shield, Heart, TrendingUp,
} from "lucide-react";
...
export function QuickActionsCard({
  ...
  if (showAskAlexi) {
    actions.push({
      ...
      sub: es ? "Ayuda de estudio personalizada" : "Get personalised study help",
      ...
      <CardContent className="flex h-full flex-col p-5">
        ...
        <div className="mt-3 flex items-start gap-3">
          ...
          <div className="min-w-0">
            <p className="text-sm font-medium" data-testid="text-current-exam-name">{label}</p>
            {isActive && (
...
export function UpgradeCard() {
  ...
  return (
    <Card className="h-full border-primary/25 bg-primary/[0.04]" data-testid="card-upgrade">
      <CardContent className="flex h-full flex-col p-5">
        <h2 className="text-base font-semibold">
          {es ? "Desbloquea el acceso completo" : "Unlock Full Access"}
        </h2>
        <p className="mt-2 text-sm text-muted-foreground">
          {es
            ? "Práctica ilimitada, recomendaciones de Alexi, exámenes simulados completos y seguimiento avanzado de tu preparación."
            : "Get unlimited practice, Alexi recommendations, full mock exams and advanced readiness tracking."}
        </p>
        <Button asChild className="mt-auto w-full sm:w-auto" data-testid="button-view-plans">
