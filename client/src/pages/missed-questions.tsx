...
export default function MissedQuestionsPage() {
  ...
  // Honour ?filter= so "Review 12 missed questions" opens that view directly.
  const search = useSearch();
  const requestedFilter = new URLSearchParams(search).get("filter") as NotebookFilter | null;
  const VALID_FILTERS: NotebookFilter[] = ["all", "struggling", "mastered", "recent"];
  const [filter, setFilter] = useState<NotebookFilter>(
    requestedFilter && VALID_FILTERS.includes(requestedFilter) ? requestedFilter : "struggling",
  );
