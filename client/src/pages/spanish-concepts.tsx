...
const HUBS = {
  seguros: ["deducible", "indemnizacion", "subrogacion", "prima", "beneficiario", "periodo-de-gracia"],
  "bienes-raices": ["agencia", "escritura-vs-titulo"],
} as const;
  ...
  return (
    <Card>
      ...
      <CardContent>
        ...
        <Link href={`/es/conceptos/${slug}`} className="text-primary font-medium hover:underline">Estudiar este concepto →</Link>
      </CardContent>
    </Card>
  );
}

export function SpanishConceptHub({ kind }: { kind: keyof typeof HUBS }) {
  const slugs = HUBS[kind];
  const isInsurance = kind === "seguros";
  const h1 = isInsurance ? "Conceptos de Seguros para el Examen de Texas" : "Conceptos de Bienes Raíces para el Examen de Texas";
  const canonical = buildUrl(`/es/conceptos/${kind}`);
  useSEO({
    title: `${h1} | MyEasyPass`,
...
export default function SpanishConceptPage() {
  const { slug } = useParams<{ slug: string }>();
  const concept = CONCEPTS[(slug ?? "") as ConceptSlug];
  if (!concept) return <div className="min-h-screen flex flex-col bg-background"><Navbar /><main className="flex-1 container mx-auto px-4 py-20"><h1 className="text-3xl font-bold">Concepto no encontrado</h1></main><Footer /></div>;

  const canonical = buildUrl(`/es/conceptos/${slug}`);
  useSEO({
    title: concept.title,
    ...
    jsonLd: [
      ...
      { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
        ...
        { "@type": "ListItem", position: 2, name: "Conceptos", item: buildUrl(concept.category === "real-estate" ? "/es/conceptos/bienes-raices" : "/es/conceptos/seguros") },
